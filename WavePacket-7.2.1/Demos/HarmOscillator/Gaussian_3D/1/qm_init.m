...
space.dof{2}.n_pts = 032;                % Number of grid points
...
space.dof{3}.n_pts = 032;                % Number of grid points
...
time.dof{1}.width = 2.0*sqrt(1/2);       % Width: squeezed state
...
plots.density           = vis.surface;   % 3D iso-surface plot
plots.density.represent = 'dvr';         % DVR (positions) or FBR (momenta)
plots.density.srf_view  = [60 45];       % View point for surface plot: [az el]
plots.density.energy    = true;          % Iso-surfaces for pot | kin energy
plots.density.srf_maps  = [true true];   % Choice of color maps or default colors
plots.density.srf_nlev  = [6 3];         % Number of iso-surfaces: density|energy
plots.density.srf_color = [true true];   % Toggle manual settings of iso-levels 
plots.density.col_min   = [0.01 10];     % Lowest iso-level for density plots
plots.density.col_max   = [0.1 20];      % Highest iso-level for density plots

% Alternative: reduced density representations
