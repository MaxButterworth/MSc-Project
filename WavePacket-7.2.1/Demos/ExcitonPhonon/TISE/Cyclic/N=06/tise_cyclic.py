...
def coupled_tise(batch_mode):
    ...
    hamilton = Coupled(
        ...
        alpha=1e-1,                      # exciton site energy
        ...
        sig=2e-4,                        # exciton-phonon tuning: symmetric
        tau=0e-4,                        # exciton-phonon coupling: pair distance
    )

    # Set up TT representation of the Hamiltonian
    hamilton.get_TT(
        n_basis=[2, 8],                  # size of exciton-phonon basis set
        qtt=False                        # using quantized TT format
    )

    # Set up solver for time-independent equation
    dynamics = TISE(
        hamilton=hamilton,               # choice of Hamiltonian, see above
        n_levels=1,                      # number of energy levels to be calculated
        solver='als',                    # choice of eigen-solver for the full system
        eigen='eigs',                    # choice of eigen-solver for the micro systems
        ranks=20,                        # rank of initial guess for ALS
        repeats=20,                      # number of sweeps in eigen-solver scheme
        conv_eps=1e-8,                   # threshold for detecting convergence of the eigenvalue
        e_est=0.0818,                    # estimation: eigenvalues closest to this number
        e_min=0.080,                     # lower end of energy plot axis (if exact energies not available!)
        e_max=0.085,                     # upper end of energy plot axis (if exact energies not available!)
    ...
    # Batch mode
    if batch_mode:
        dynamics.solve()                 # Solve equation *without* visualization
    ...
    # Interactive mode: Setup animated visualization
        ...
        graphics = VisualTISE(
            ...
            plot_type='QuantDisplace2',  # select your favorite plot type
        ...
        graphics.solve()                # Solve equation *with* visualization
