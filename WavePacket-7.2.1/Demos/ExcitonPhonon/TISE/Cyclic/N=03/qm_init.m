...
for d = 1:N
    ...
    space.dof{d}.n_pts = 16;             % Number of grid points / basis functions
...
% QM_BOUND only: Choice of eigenstates
hamilt.eigen.start = 000;
hamilt.eigen.stop  = 005;
hamilt.eigen.storage = 's';
...
plots.density.srf_maps  = [0 0];         % Choice of color maps or default colors
plots.density.srf_nlev  = [2 3];         % Number of iso-surfaces: density|energy
plots.density.srf_color = [true true];   % Toggle manual settings of iso-levels 
plots.density.col_min   = [1e-7 0.10];   % Lowest iso-level for density plots
plots.density.col_max   = [1e-6 0.11];   % Highest iso-level for density plots
