...
if isfield (plots,'density')
    ...
    % First call only
        ...
        if plots.density.export 
            ...
            if ~plots.density.images && strcmpi(info.system,'Matlab')
                ...
                writerObj.FrameRate = 2; % Rate of video playback in frames per second
                open(writerObj);
            else
    ...
    if isfield (plots,'expect')
        ...
        % Draw textbox with trajectory details
        if step==1
    ...
    if step==1 && strcmpi(info.system,'Matlab')
        ...
        prt.disp (['Rate of playback in frames per second : ' int2str(writerObj.FrameRate)])
        prt.disp (' ')
    end
