...
classdef interp < handle
    ...
    properties (Access = public)
        ...
        mom_0       % If set, apply momentum: multiply with exp(ikr)
    end
    ...
    methods (Access = public)
        ...
        function disp(obj)
            ...
            prt.disp (['Interpolation method              : '         obj.method])
             if space.n_dim>1
                prt.disp (['Number of data points in each dof : ' int2str(obj.n_pts)])
            end
            if ~isempty(obj.mom_0)
                prt.disp (['Shift in wave fcts momentum space : ' num2str(obj.mom_0)])
            end
       end
        ...
        function wave (obj, psi)
            ...
            % Optionally shift (coupled) wavefunction(s) in momentum space
            if ~isempty(obj.mom_0)
                shift_mom (state, obj.mom_0)
            end
