...
prt.disp('***************************************************************');
prt.disp('Loading state vectors from mat-file')
prt.disp('***************************************************************');
prt.disp(' ');
prt.disp(['Directory/folder   : ' prt.shorten(state.save_dir)  ])
prt.disp(['File name (binary) : ' prt.shorten(file_name) ])
prt.disp(['Index of this step : ' int2str(step_indx) ])
prt.disp(' ');

% Load trajectory arrays from data file
load(fullfile(state.save_dir,file_name), 'arr_x', '-mat');
state.x = arr_x(:,step_indx);
