...
classdef pendulum < pot.generic & handle
    properties (Access = public)
        ...
        v_0         % energy shift
        omg         % harmonic oscillator frequencies
    ...
    methods (Access = public)
        ...
        function init (obj)
            global space
            if space.n_dim>1 
                if space.n_dim-1 ~= length(obj.omg)
                    prt.error ('Wrong number of harmonic oscillator frequencies')
                end
            end
         end
        ...
        function disp(obj)
            ...
            if space.n_dim>1
                prt.disp ('                                                  ')
                prt.disp ('Harmonic oscillators in remaining dof-s          ')
            end
            for dof=2:space.n_dim
                prt.disp ([int2str(dof) '-th dof with frequency : ' num2str(obj.omg(dof-1))])
            end
            prt.disp (' ')
        end
        ...
        function V = V(obj,r)
            ...
            % Evaluate potential energy function of generalized pendulum
            V = ...
                + obj.xi*csc2 ...
                + obj.chi*csc2.*cos(x) ...
                - obj.eta*cos(x) ...
            ...
            % Optionally add potential energy function of harmonic oscillator(s)
            for dof=2:space.n_dim
                omega = obj.omg(dof-1);
                mass = space.dof{dof}.mass;
                V = V + mass * omega^2 * r{dof}.^2/2;
            end
