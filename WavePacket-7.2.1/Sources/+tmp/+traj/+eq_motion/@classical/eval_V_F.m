...
if(strcmpi(hamilt.coupling.represent,'adi'))
    ...
    if(~init && state.extra_tdse_solving)
        psi = solve_tdse_adi (obj, state, step_size, state.mom, state.pot_mat, state.frc_mat, ...
                                    ...
                                    state.psi, state.psi_old); % adiabatic
    ...
    for m = 1:hamilt.coupling.n_eqs
        ...
        if (~isempty(ind_m))
            ...
            frc_adi = eq_frc (time.eq_motion, pos_ind_m, mom_ind_m, state.pot_mat(:,:,ind_m), ...
                    frc_mats_ind_m, state.D_new(:,ind_m), state.U_new(:,:,ind_m), m, psi_ind_m);
...
% Diabatic representation
    ...
    if (~init)
        psi  = solve_tdse_dia (obj, state, step_size, state.pos, state.psi, state.psi_old); % diabatic
    ...
    for m = 1:hamilt.coupling.n_eqs
        ...
        if ~isempty(r_m)
            ...
            frc_dia_m = eq_frc_dia (time.eq_motion, r_m, m , psi_ind_m);
            
            % Save corresponding potentials and forces
            state.pot (ind_m) = V_m;
            for d=1:space.n_dim
                state.frc{d}(ind_m) = frc_dia_m{d};
            end
        end
    end
