...
% as well D_p(R), D_t(R), and P(R) can have several cartesian 
% components corresponding to different polarization directions.
%
...
classdef strang < tmp.wave.trotter & handle
    ...
    methods (Access = public)
        ...
        function propa (obj, psi)
            ...
            % Same as in Trotter, but using frac=1/2 for kinetic operator
            propa@tmp.wave.trotter (obj, psi)
            ...
            % Interaction with external electric field through
            ...
            if isfield(time, 'pulse')
                ...
                % If possible, already use field at next time sub-step
                if time.steps.counter < length (time.efield.grid{1})
                    for p=1:time.efield.n_polar
                        efield(1,p) = time.efield.grid{p}(time.steps.counter+1);
                    end
                else
                    for p=1:time.efield.n_polar
                        efield(1,p) = time.efield.grid{p}(time.steps.counter);
                    end
                end

                % Permanent/transition dipoles, polarizabilities: reversed
                pol_propa   (obj, psi, efield);
                trans_propa (obj, psi, efield);
                perm_propa  (obj, psi, efield);
            end
