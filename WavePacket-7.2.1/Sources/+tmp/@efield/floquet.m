...
for p = 1:length(hamilt.dip)                     % Dipole components for bare states
        ...
        for n = 1:bare.coupling.n_eqs
            for phn = 1:length(obj.photons{n})
                ...
                if photonsleft == photonsright
                    ...
                    hamilt.pot{indexleft, indexright} = pot.generic;
                end
            end
        end
    end
        ...
        for n = 1:bare.coupling.n_eqs
            for phn = 1:length(obj.photons{n})
                ...
                for p = 1:length(hamilt.dip)
                    if ~isempty (hamilt.dip{p})
                        if ~bare.dip{p}{m,n}.empty && abs(photonsleft-photonsright) == 1
                            ...
                            hamilt.dip{p}{indexleft, indexright} = dip.generic;
                        end
                    end
                end
