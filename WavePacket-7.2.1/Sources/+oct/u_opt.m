...
if ~isempty(state.C) && state.Q{control.optimal.terminal}
    switch lower (control.optimal.prefactor)
        ...
        otherwise
            prt.error(['Wrong choice for prefactor time : ' control.optimal.prefactor])
    end
...
switch lower(action)
    ...
    otherwise
        prt.error (['Calling u_opt function with wrong "ACTION" keyword: ' action])
end
