...
function pot_eig_2 ( efield, save_trafo )
global hamilt space
...
if ~hamilt.pot{2,2}.empty 
    gamma = hamilt.pot{2,2}.dvr;
else % free particle
    gamma = zeros ( size ( space.dvr{1} ) );
end

% Permanent dipole moments along all polarization directions
if isfield(hamilt, 'dip') && ~isempty(efield)
    for p = 1:length(hamilt.dip)
        if abs(efield(p))>0
            if ~isempty (hamilt.dip{p})
                if ~hamilt.dip{p}{1,1}.empty
                    alpha = alpha - efield(p) * hamilt.dip{p}{1,1}.dvr;
                end
                if ~hamilt.dip{p}{2,2}.empty
                    gamma = gamma - efield(p) * hamilt.dip{p}{2,2}.dvr;
                end
            end
        end
    end
                ...
                if abs(efield(q))>0
                    if ~isempty (hamilt.pol{p,q})
                        if ~hamilt.pol{p,q}{1,1}.empty
                            alpha = alpha - efield(p)*efield(q)/2 * hamilt.pol{p,q}{1,1}.dvr;
                        end
                        if ~hamilt.pol{p,q}{2,2}.empty
                            gamma = gamma - efield(p)*efield(q)/2 * hamilt.pol{p,q}{2,2}.dvr ;
                        end
                    end
                end
            end
...
if ~hamilt.pot{1,2}.empty 
    beta  =  beta + hamilt.pot{1,2}.dvr;
end

% Transition dipole moments along all polarization directions
if isfield(hamilt, 'dip') && ~isempty(efield)
    for p = 1:length(hamilt.dip)
        if abs(efield(p))>0
            if ~isempty (hamilt.dip{p})
                if  ~hamilt.dip{p}{1,2}.empty
                    beta = beta - efield(p) * hamilt.dip{p}{1,2}.dvr;
                end
            end
        end
    end
    ...
    for p = 1:size(hamilt.pol,1)
        if abs(efield(p))>0
            for q = p:size(hamilt.pol,2)
                if abs(efield(q))>0
                    if ~isempty (hamilt.pol{p,q})
                        if  ~hamilt.pol{p,q}{1,2}.empty
                            beta = beta - efield(p)*efield(q)/2 * hamilt.pol{p,q}{1,2}.dvr;
                        end
                    end
                end
            end
