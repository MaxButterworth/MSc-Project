...
function nac_norm_all = nac_norm_all (pot_mats,frc_mats,U,D)
global space hamilt
...
if n_traj>0
    ...
    nac_norm_all = cell(hamilt.coupling.n_eqs,hamilt.coupling.n_eqs,space.n_dim);
    ...
    switch hamilt.coupling.n_eqs
        case 1
            for m=1:hamilt.coupling.n_eqs
                for n=1:hamilt.coupling.n_eqs
                    for d=1:space.n_dim
                        nac_norm_all{m,n,d} = zeros(n_traj,1);
                    end
                end
            end
        ...
        otherwise
            ...
            for m=1:hamilt.coupling.n_eqs
                for d=1:space.n_dim
                    nac_norm_all{m,m,d} = zeros(n_traj,1);
                end
                
                for n=(m+1):hamilt.coupling.n_eqs
                    
                    nac_norm_mn = ham.nac_norm_mn (pot_mats,frc_mats,U,D,m,n);
                    
                    for d=1:space.n_dim
                        nac_norm_all{m,n,d} =   nac_norm_mn{d};
                        nac_norm_all{n,m,d} = - nac_norm_mn{d};
                    end
                end
            end
    end
