...
function probable = lz_formula(obj, mom, dia_pot_mats, dia_frc_mats, U, D, gap, gap_d2, m, n, lz_variant)
global hamilt space

if nargin==10
    lz_variant = obj.lz_variant;
end
...
switch lz_variant
    ...
    case 7
        % Compute super-adiabatic (sa) eigenvectors
        u_sa = ham.pot_eig_sa(mom, dia_pot_mats, dia_frc_mats, D, U);
        
        % Compute coupling between the m-th and n-th sa eigenvector
        um_un  = zeros (length(mom{1}),1);
        for k = 1 : hamilt.coupling.n_eqs
            um_un(:) = um_un + squeeze( abs(u_sa(k,m,:)) .* abs(u_sa(k,n,:)) );
        end
        
        % Compute LZ_2 formula
        lz_2 = lz_formula(obj, mom, dia_pot_mats, dia_frc_mats, U, D, gap, gap_d2, m, n, 2);
        
        % Compute hop probability
        probable = um_un .* lz_2;
