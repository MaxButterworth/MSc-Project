...
if index.type == '.'
    switch index.subs
        case 'dof'
            if length(val) ~= 2
                prt.error('Operator needs to operate on two dimensions');
            end
            obj.dof = val;
        case 'mass'
            if length(val) ~= 3
                prt.error('Operator needs three masses for setup');
            end
            obj.mass = val;
        case 'theta'
            obj.theta = val;
        otherwise
            prt.error ( ['Tried to set invalid field ' index.subs 'in kinetic object'] );
    end
end
