...
switch lower(obj.choice_eq_motion)
    ...
    case {'bh'} % quantum trajectories: adiabatic
        time.eq_motion = tmp.traj.eq_motion.Born_Huang;
        
    case {'qt_dia'} % quantum trajectories: diabatic
        time.eq_motion = tmp.traj.eq_motion.quantraj_dia;
        
    case {'qt_dia_mod'} % quantum trajectories: diabatic
        time.eq_motion = tmp.traj.eq_motion.quantraj_dia_mod;
        
    case {'qt_adi_2'} % quantum trajectories: adiabatic
        time.eq_motion = tmp.traj.eq_motion.quantraj_adi_2;
        
    case {'qt_adi'} % quantum trajectories: adiabatic
        time.eq_motion = tmp.traj.eq_motion.quantraj_adi_L;
        
    case {'qt_adi_mod'} % quantum trajectories: adiabatic
        time.eq_motion = tmp.traj.eq_motion.quantraj_adi_L_mod;
        
    case {'qt_adi_ext'} % quantum trajectories: adiabatic
        time.eq_motion = tmp.traj.eq_motion.quantraj_adi_L;
        time.eq_motion.add_bmm = 1;
        
    case {'qt_adi_mod_ext'} % quantum trajectories: adiabatic
        time.eq_motion = tmp.traj.eq_motion.quantraj_adi_L_mod;
        time.eq_motion.add_bmm = 1;
        
    case 'super_adi' % classical trajectories
        time.eq_motion = tmp.traj.eq_motion.super_adi;
        
    case 'qt_adi_corr' % classical trajectories
        time.eq_motion = tmp.traj.eq_motion.quantraj_adi_corr;
...
if hamilt.coupling.n_eqs>1
    ...
    if ~isempty(hamilt.coupling.ini_coeffs)
        ...
        if strcmpi(hamilt.coupling.represent,hamilt.coupling.ini_rep)
            ...
            obj.pot_mat = ham.pot_dia(obj.pos);
            ...
            for t = 1:obj.n_p
                ...
                if strcmpi(hamilt.coupling.represent,'adi')
                    init_rep   = U' * hamilt.coupling.ini_coeffs';
                    obj.cha(t) = randomPop ( init_rep );
